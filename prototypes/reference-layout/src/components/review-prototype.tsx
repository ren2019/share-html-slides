...
export function ReviewPrototype({children,initialVersion}:{children:ReactNode;initialVersion:number}){
 ...
 {quote&&!open&&canWrite&&<button className="rp-selection rp-primary" onMouseDown={e=>e.preventDefault()} onClick={()=>{readingY.current=window.scrollY;setOpen(true);setPicking(false);requestAnimationFrame(()=>input.current?.focus())}}>评论选中文字</button>}
 {variant==='A'&&<div className="rp-dock">{controls}</div>}
 ...
 <PrototypeSwitcher variant={variant} onBeforeSwitch={()=>{setOpen(false);setPicking(false);setQuote('')}}/>
 </div>;
}
