...
export function config() {
  ...
  const smsEnabled = Boolean(smsURL && smsToken);
  if (process.env.NODE_ENV === 'production') {
    if (deploymentURL(baseURL).protocol !== 'https:') throw new Error('Production APP_URL must use HTTPS');
    if (smsEnabled && deploymentURL(smsURL!).protocol !== 'https:') throw new Error('Production SMS gateway must use HTTPS');
  }
  ...
  return { smtp, emailEnabled, baseURL, secret, databaseURL: process.env.DATABASE_URL, smsURL, smsToken, smsEnabled };
}
