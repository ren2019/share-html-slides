...
export default defineConfig({
  ...
  webServer: [
    ...
    { command: 'pnpm db:migrate && pnpm exec tsx tests/reset-database.ts && pnpm dev --hostname 127.0.0.1 --port 3210', url: 'http://127.0.0.1:3210', env: { ...shared, APP_URL: 'http://127.0.0.1:3210', SMS_GATEWAY_URL: 'http://127.0.0.1:3212/send', SMS_GATEWAY_TOKEN: 'integration-only' }, reuseExistingServer: false },
    { command: 'pnpm dev --hostname 127.0.0.1 --port 3211', url: 'http://127.0.0.1:3211', env: { ...shared, APP_URL: 'http://127.0.0.1:3211', NEXT_DIST_DIR: '.next-disabled', SMS_GATEWAY_URL: '', SMS_GATEWAY_TOKEN: '' }, reuseExistingServer: false },
  ],
});
